...
PRIVCONST_DECLARATION = re.compile(r'@privconst\s+(\w+)\s+(\w+)(\s*=\s*([^;]+))?\s*;', flags=re.IGNORECASE) 
CONST_DECLARATION = re.compile(r'@const\s+(\w+)\s+(\w+)(\s*=\s*([^;]+))?\s*;', flags=re.IGNORECASE) 
VAR_DECLARATION = re.compile(r'@(\w+)\s+(\w+)(\s*=\s*([^;]+))?\s*;', flags=re.IGNORECASE) 
...
def find_global_vars():
    ...
    for filename in files:
        ...
        for match in VAR_DECLARATION.finditer(input):
            var = Var()
...
def write_global_vars(out, ints, strs, type):
...
def replace_var_uses(input, vars):                
    def repl(match):
        name = match.group(1)
    ...
    return VAR_USE.sub(repl, input)
   
   
def remove_variable_declarations(input):
    input = VAR_DECLARATION.sub('', input)
    return input
    
def replace_constants_acc(input):
    input = CONST_DECLARATION.sub(r'#libdefine \2 \4', input)
    input = PRIVCONST_DECLARATION.sub(r'#define \2 \4', input)
    return input
    
def replace_constants_bcc(input):
    input = CONST_DECLARATION.sub(r'enum : \1 { \2 = \4 };', input)
    input = PRIVCONST_DECLARATION.sub(r'private enum : \1 { \2 = \4 };', input)
    return input
    
# Type replacement
def remove_casts(input):
    return re.sub(r'\b(int|fixed|str|bool|ang)\(', '(', input)

def replace_casts(input):
    return re.sub(r'\b(int|fixed|str|bool|ang)\(', r'(\1)(', input)
   
def replace_types_acc(input):
    for a, b in TYPE_ALIASES_ACC.items():
        input = re.sub(r'\b' + a + r'\b', b, input)
    return input
   
def replace_types_bcc(input):
    for a, b in TYPE_ALIASES_BCC.items():
        input = re.sub(r'\b' + a + r'\b', b, input)
    return input
   
def main():
    vars = find_global_vars()
    ints, strs = allocate_global_vars(vars)
    ...
    for filename in files:
        input = readfile('src/' + filename, 'r', encoding='utf-8')
        input = replace_var_uses(input, vars)
